...
import { OutgoingHttpHeaders, RequestOptions as HTTPRequestOptions, RequestOptions as HTTPSRequestOptions } from "http";
...
function getSignedHeaders(headers: OutgoingHttpHeaders) {
  if (!isObject(headers)) {
...
function getSigningKey(date: Date, region: string, secretKey: string) {
  if (!isObject(date)) {
  ...
  var dateLine = makeDateShort(date),
    hmac1 = Crypto.createHmac("sha256", "AWS4" + secretKey)
      .update(dateLine)
      .digest(),
    hmac2 = Crypto.createHmac("sha256", hmac1)
      .update(region)
      .digest(),
    hmac3 = Crypto.createHmac("sha256", hmac2)
      .update("s3")
      .digest();
  return Crypto.createHmac("sha256", hmac3)
    .update("aws4_request")
    .digest();
}
...
export function postPresignSignatureV4(
  region: string,
  date: Date,
  secretKey: string,
  policyBase64: string
...
export function signV4(
  ...
  var sha256sum = request.headers!["x-amz-content-sha256"] as string;
...
export function presignSignatureV4(
  ...
  if (expires < 1) {
    throw new errors.ExpiresParamError(
